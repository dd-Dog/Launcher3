...
import org.json.*;
/* SPRD: BugFix bug343227 2014-08-15 don't add the shortcut if the app is not available. @{ */
...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent data) {
        ...
        /* SPRD: fix bug 333996, add toast for shortcut installed @{ */
        if(name != null){
    ...
    static void flushInstallQueue(Context context) {
        ...
        if (!installQueue.isEmpty()) {
            ...
            while (iter.hasNext()) {
                ...
                if (cn != null) {
                    resolveInfos = AllAppsList.findActivitiesForPackage(context, cn.getPackageName());
                } else {
                    resolveInfos = AllAppsList.findActivitysByIntent(context, intent);
                }
                boolean isValidPackageComponent = (resolveInfos != null && resolveInfos.size() > 0) ? true : false;

                // TODO-XXX: Disable duplicates for now
                if (!exists && isValidPackageComponent /* && allowDuplicate */) {
