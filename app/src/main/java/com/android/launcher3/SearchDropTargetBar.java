...
/* SPRD: bug339563 mQSBSearchBar and mDropTargetBar display overlapping. @{ */
...
public class SearchDropTargetBar extends FrameLayout implements DragController.DragListener {
    ...
    /* SPRD: bug339563 mQSBSearchBar and mDropTargetBar display overlapping. @{ */
    private String TAG = "Launcher.SearchDropTargetBar";
    /* SPRD: bug339563 mQSBSearchBar and mDropTargetBar display overlapping. @} */
    ...
    private void setupAnimation(ObjectAnimator anim, final View v) {
        ...
        anim.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                /* SPRD: bug339563 mQSBSearchBar and mDropTargetBar display overlapping. @{ */
                if (v.getId() == R.id.drag_target_bar) {
                    Log.d(TAG, String.format("onAnimationEnd mDropTargetBar.getAlpha()=%f", v.getAlpha()));
                } else {
                    Log.d(TAG, String.format("onAnimationEnd mQSBSearchBar.getAlpha()=%f", v.getAlpha()));
                }
                /* SPRD: bug339563 mQSBSearchBar and mDropTargetBar display overlapping. @} */
            }
        });
    ...
    public void onDragEnd() {
        /* SPRD: bug339563 mQSBSearchBar and mDropTargetBar display overlapping. @{ */
        Log.d(TAG, String.format("onDragEnd() mDeferOnDragEnd=%b mIsSearchBarHidden=%b\n%s",
                mDeferOnDragEnd, mIsSearchBarHidden, Log.getStackTraceString(new Throwable())));
        if (!mDeferOnDragEnd) {
            // Restore the QSB search bar, and animate out the drop target bar
            if (mDropTargetBarAnim.isRunning()) {
                mDropTargetBarAnim.cancel();
            }
            prepareStartAnimation(mDropTargetBar);
            mDropTargetBarAnim.reverse();
            if (!mIsSearchBarHidden) {
                if (mQSBSearchBarAnim.isRunning()) {
                    mQSBSearchBarAnim.cancel();
                }
                prepareStartAnimation(mQSBSearchBar);
                mQSBSearchBarAnim.reverse();
